...
void conduct()
{
	...
	len1 = getdis(p[0], p[n-1]); v1 = p[n-1] - p[0]; v[0] = (vec){ 0, 0 };
	for (i = 1; i < n; ++i) v[i] = uvec(p[i]-p[0]) * getdis(p[i], p[0]) / len1;
...
//	printf("l --> "); for (i = 0; i < n; ++i) printf("%.3f ", len[i]); printf("\n");
	for (lst = p[0], mst = p[n-1], i = 0; i < itr; ++i) {
		...
		mst = lst + rotate(v[j], v1, tv) * tl; lst = lst + rotate(v[j-1], v1, tv) * tl;
