...
void conduct() {
	int i, tmp, tmp2;
	scanf("%d%d", &m, &n);
	for (i=0; i<m; ++i) { x[i]=-1; scanf("%d", &y[i]); }
	memset(way, 0, sizeof(way)); memset(hd, 0, sizeof(hd)); cnt=0;
	for (i=0; i<n; ++i) {
		scanf("%d", &tmp);
		while (tmp--) {
			scanf("%d", &tmp2); --tmp2;
			if (!~x[tmp2]) { insert(ss, i, y[tmp2]); insert(i, ss, 0); }
			else { insert(x[tmp2], i, INF_FLOW); insert(i, x[tmp2], 0); }
			x[tmp2]=i;
		}
		scanf("%d", &tmp);
		insert(i, tt, tmp); insert(tt, i, 0);
	}
