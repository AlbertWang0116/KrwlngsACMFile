...
void add(int l, int r, int x, int y, void (*pref)(SegNode), void (*pstf)(SegNode), void (*ctlf)(int, int, int)) {
...
int check(int l, int r, int x, int y, void (*pref)(SegNode), int (*ctlf)(int, int, int)) {
...
void pref(SegNode v) { pre(v.a, v.b, v.c, v.d, v.e, v.f); } 
void pstf(SegNode v) { sum[v.a] = sum[v.b] + sum[v.c]; }
void addf(int id, int ll, int rr) { csh[id]++; sum[id] += rr - ll; };
int sumf(int id, int ll, int rr) { return sum[id]; };
